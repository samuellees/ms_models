    ...
    def eval_training(self):
        ...
        with torch.no_grad():
            for (inputs, labels) in self.dataloader_test:
                ...
                val_acc_megvii += prec1

        time_finish = time.time()
        val_acc_megvii /= total_samples
        val_acc = correct_samples / total_samples
        avg_loss = test_loss / len(self.dataloader_test)
        if self.local_rank == 0:
            logging.info('Test set: Average loss: {:.4f}, Accuracy1: {:.4f}, Time consumed:{:.2f}s'.format(
                avg_loss, val_acc, time_finish - time_start
            ))
            ...
            self.summary_writer.add_scalar('Test/MegVIIAccuracy', val_acc_megvii, self.epoch_id)
        return val_acc
